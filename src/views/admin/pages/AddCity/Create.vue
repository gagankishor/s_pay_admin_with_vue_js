...
export default {
  ...
  methods: {
    async add_city() {
      ...
      this.loading = true;
      let apiResponse = await ApiExecute("city", "POST", {
        data: formData,
      });
      this.loading = false;
    ...
    async deletecity(id){
            console.warn(id)
            let apiResponse = await ApiExecute(`city/${id}`, "DELETE");
            if(apiResponse.status){
              this.gettable(1);
    ...
    async gettable(page = 1) {
      ...
      let apiResponse = await ApiExecute(
        `city/?page=${page}&limit=${this.limit}&search=${this.search}`
      );
      this.loading = false;
